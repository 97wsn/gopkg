...
func TestParseDateTime(t *testing.T) {
	...
	date2 := "2020-12-21T10:58:37+08:00"
	tm, err = ParseDateTime(date2)

	tm2, err2 := time.Parse(time.RFC3339, date2)
	if err != nil || err2 != nil || tm == tm2 {
		t.Fatalf("parse datetime RFC3339 error")
	}
}
...
func TestFormatDateIgnoreZero(t *testing.T) {
	testDate := time.Time{}

	if FormatDateIgnoreZero(testDate) != "" {
		t.Fatalf("format date ignore zero error")
	}

	testDate2 := "2012-10-24"
	tm, _ := ParseDate(testDate2)

	if FormatDateIgnoreZero(tm) != testDate2 {
		t.Fatalf("format date error")
	}
}
...
func TestFormatDateTimeIgnoreZero(t *testing.T) {
	testDate := time.Time{}

	if FormatDateTimeIgnoreZero(testDate) != "" {
		t.Fatalf("format datetime ignore zero error")
	}

	testDate2 := "2012-10-24 07:21:15"
	tm, _ := ParseDateTime(testDate2)

	if FormatDateTimeIgnoreZero(tm) != testDate2 {
		t.Fatalf("format datetime ignore zero error")
	}
}

func TestFormatDateTimeMilli(t *testing.T) {
	testDate := "2012-10-24 07:21:15.001"
	tm, err := ParseDateTime(testDate)

	if err != nil || FormatDateTimeMilli(tm) != testDate {
		t.Fatalf("format date error")
	}
}

func TestFormatDateTimeMilliIgnoreZero(t *testing.T) {
	testDate := time.Time{}

	if FormatDateTimeMilliIgnoreZero(testDate) != "" {
		t.Fatalf("format datetime ignore zero error")
	}

	testDate2 := "2012-10-24 07:21:15.001"
	tm, _ := ParseDateTime(testDate2)

	if FormatDateTimeMilliIgnoreZero(tm) != testDate2 {
		t.Fatalf("format datetime ignore zero error")
	}
}
...
func TestToUnix(t *testing.T) {
	...
	if ToUnix("this is test") != 0 {
		t.Fatalf("to unix zero error")
	}
...
func TestSplitTime(t *testing.T) {
	...
	endTime, err = ParseDateTime("2018-08-01 00:59:59")
	assert.NoError(t, err)
	res = SplitTime(startTime, endTime, 2*time.Hour)
	l = len(res)
	if l != 1 {
		t.Errorf("time split error,want splited length 1,got %d \n", l)
	}
	if res[0].EndTime.Format("2006-01-02 15:04:05") != "2018-08-01 00:59:59" {
		t.Error("time split item error")
	}
}
...
func TestLocTime(t *testing.T) {
	tt, _ := time.ParseInLocation("2006-01-02 15:04:05", "2012-10-24 07:09:00", time.UTC)
	got := LocTime(tt).Format("2006-01-02 15:04:05")
	assert.Equal(t, "2012-10-24 15:09:00", got)
}

func TestOptional(t *testing.T) {
	tt, _ := time.ParseInLocation("2006-01-02 15:04:05", "2012-10-24 07:09:00", loc)
	assert.Equal(t, "2012-10-24 07:09:00", FormatDateTime(Optional(&tt)))
}
